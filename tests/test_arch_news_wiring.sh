...
for workflow_line in "${workflow_lines[@]}"; do
    ...
    workflow_positions+=(
        "$(line_number "$workflow_line")"
    )
done

for ((\
index = 1;  \
index < ${#workflow_positions[@]};  \
index++)); do
    previous_index=$((index - 1))

    if ((\
        workflow_positions[previous_index] >= \
        workflow_positions[index])) \
            ; then
